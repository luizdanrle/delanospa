...
export default function NewTherapistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Inserting therapist data:', insertData)
      
      const { data, error } = await supabase.from('therapists').insert([insertData]).select()
      ...
      await supabase.from('therapists').select('*').limit(1)
      
      alert('Massagista criado com sucesso!')
      router.push('/admin/dashboard?tab=therapists')
      router.refresh()
    } catch (error: any) {
      console.error('Error creating therapist:', error)
      alert(`Erro ao criar massagista: ${error.message || 'Verifique se a tabela existe no Supabase'}`)
    } finally {
      setLoading(false)
    }
  }
      ...
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800">
        <div className="flex items-center justify-between px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white">Nova Massagista</h1>
          </div>
        </div>
      </header>
