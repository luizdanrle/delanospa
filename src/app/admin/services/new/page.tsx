...
export default function NewServicePage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'image_url' | 'gallery') => {
    ...
    try {
      ...
      if (uploadError) {
        console.warn('Supabase storage error (bucket pode não existir):', uploadError.message)
        // Fallback: usar URL temporária ou serviço externo
        finalUrl = tempUrl
        // Mostrar aviso ao usuário
        alert('⚠️ Bucket de armazenamento não configurado. A imagem será usada temporariamente.\n\nPara configurar o bucket:\n1. Vá ao painel do Supabase\n2. Storage → New bucket\n3. Crie bucket "media" com acesso público')
      } else {
